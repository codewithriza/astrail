...
Use the root application tree. Put import and generation logic in `lib/`, runtime behavior in `lib/runtime/`, route adapters in `app/api/`, and small reproducible examples in `examples/`. Preserve the security invariants in [engineering standards](docs/ENGINEERING.md).
